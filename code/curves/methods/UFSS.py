...
if __name__ == '__main__':
    ...
    bkps_dic = []
    bkps_truth_dic = []
    thresholds = np.linspace(0.3e5, 0.8e5,50)
    for th_index, th in enumerate(thresholds):
        ...
        for i in range(100):
            ...
            p.apply_async(UFSS, args=(X,Z,20,th,), callback=add_bkps)
            # print(bkps)
            # bkps_dic.append(bkps)
        p.close()
        p.join()
        np.save('curves/D2_UFSS/bkps_dic_'+str(th_index)+'.npy',bkps_dic)
        np.save('curves/D2_UFSS/bkps_truth_dic_'+str(th_index)+'.npy',bkps_truth_dic)
            #保存
