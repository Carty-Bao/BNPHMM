...
if __name__ == '__main__':
    ...
    for th_index, th in enumerate(thresholds):
        ...
        for i in range(10):
            ...
            p.apply_async(BNPHMMFSS, args=(X,Z,20,th,None,), callback=add_bkps)
            # print(bkps)
            # bkps_dic.append(bkps)
        p.close()
        p.join()
        np.save('curves/D2_BNPHMMF/bkps_dic_'+str(th_index)+'.npy',bkps_dic)
        np.save('curves/D2_BNPHMMF/bkps_truth_dic_'+str(th_index)+'.npy',bkps_truth_dic)
