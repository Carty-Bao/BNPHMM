...
def signal(N, f1, f2, sigma=0, start=1):
    '''
    生成采样信号
...
if __name__ == '__main__':
    ...
    #显示预测结果
    plt.plot(data, c='r',label="原始信号")
    plt.title("AR模型拟合正弦函数", fontproperties="Songti SC", fontsize=15)
    sums = 0.0
    predict_data = []
    ...
    plt.plot(predict_data[2:], c = 'g',label='AR模型预测')
    plt.xlabel("脉冲索引", fontproperties="Songti SC", fontsize=15)
    plt.ylabel("脉冲PRI值", fontproperties="Songti SC", fontsize=15)
    plt.legend(loc = "best",prop={'family' : 'Songti SC', 'size'   : 15})
    plt.show()
