...
class CUSUM_BNP_HMM(object):
    ...
    def update(self, x):
        ...
        threshold = self.calculate_threshold(ave, ave+self.changepoint_th)
        if maxS_kt > abs(self.changepoint_th):
            self.bkp_detect.append(len(self.ts))
            self.ts_temp = []
            self.s_kt = []
            return 10
        return 0    
...
if __name__ == '__main__':
    ...
    start = time.time()
#################################################################   ChangeFinder    ###################################################
    CF = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, changepoint_th=300)
