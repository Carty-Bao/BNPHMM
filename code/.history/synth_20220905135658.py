...
def PRI2TOA(PRI_signal):
    ...
    for index in range(len(PRI_signal)):
        TOA.append(TOA_init[index] - TOA_init[0])
        # TOA = np.cumsum(PRI_signal_init)
    return TOA, init_TOA

def TOA2PRI(TOA_signal):
    '''
    将一个数据序列从TOA转变为PRI
    '''
    for index in range(len(TOA_signal)):
        TOA_signal[index] += self.init_TOA
...
def add_spur_PRI(PRI_signal, spur_ratio):
    ...
    #计算虚假脉冲个数
    spur_num = math.floor(para * len(TOA_signal))
...
def miss_PRI(PRI_signal, miss_ratio):
    '''
    缺失脉冲
