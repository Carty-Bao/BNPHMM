...
for i in range(batch_num):
    ...
    #如果参差中心不同并且上一个minbatch不是切换点
        if center_compare(minibatch_mu[-1], minibatch_mu[-2],threshold = 10)[0] and ~initseq:
...
CF_p = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, threshold=1e2)
...
for index in range(1,len(bkps)):
    plt.plot(range(bkps[index-1]+1, bkps[index]),X[bkps[index-1]+1: bkps[index]], marker="*", color=bank[index])
# plt.savefig("draw/D3.pdf",dpi=300)
# %%
