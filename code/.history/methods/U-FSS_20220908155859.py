...
def UFSS(X, Z, batchsize = 20, FSS_threshold = 5):
    ...
    start = time.time()
    bkps=[]
    CF = FSS(X, bkps=[], mean=[], var=[], para_known=False, fixed_threshold=FSS_threshold, fixed_size=20)#高斯抖动为800
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        Z = np.array([D1[:,1]]).T
        #推理
        bkps = BNPHMMCUSUM(X, Z, CUSUM_threshold=5, unideal_threshold=None)
