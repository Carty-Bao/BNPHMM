...
class DP_GMM(object):
    ...
    def update_m_sigma(self,randInit=0):
        ...
        vk = v0 + NK + 1
        xd = self.calc_xk(self.exp_z,self.X)
        S = self.calc_S(self.exp_z,self.X,xd,NK)
        betak = beta0 + NK
        self._m = self.update_mu(K,XDim,beta0,m0,NK,xd,betak)
        self._W = self.update_W(K,self._expW0,xd,NK,m0,XDim,beta0,S) 
        self._xd = xd
        self._S = S
        self._NK = NK
        self._vk = vk
    ...
    def update_mu(self,K,XDim,beta0,m0,NK,xd,betak):
        m = np.zeros((K,XDim))
        for k in range(K): m[k,:] = (beta0*m0 + NK[k]*xd[k,:]) / betak[k]
        return m  

    def update_W(self,K,W0,xd,NK,m0,XDim,beta0,S):
