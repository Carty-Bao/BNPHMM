...
from synth import workmode_cat, center_compare, trans_compare
from scipy.stats import entropy as entropy
from viterbi import viterbiLog
from ChangeFinder import CUSUM_BNP_HMM
...
for i in range(batch_num):
    ...
    if initseq==True:
        ...
        mean, Z, A = model.del_irr(threshold=0.4)
        minibatch_mu.append(mean)
        minibatch_a.append(A)
        minibatch_z.append(Z)
    #如果参差中心不同并且上一个minbatch不是切换点
        if center_compare(minibatch_mu[-1], minibatch_mu[-2],threshold = 5)[0] and ~initseq:
...
for index in range(1,len(bkps)):
    ...
    plt.savefig("draw/D6_miss_4.pdf",dpi=300)
...
CF_p = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, threshold=1e9)
...
while i < N:
    ...
    if initseq==True:
        ...
        model = DP_GMM(X = initbatch, K=L,Z=Znitbatch, agile=True)
        model.init_q_param()
        model.mixture_fit()
        model.HMM_fit()
        ...
        if scor > 1 :
            ...
            del model
            continue
        if trans_compare(A_1 = init_A , A_2 = minibatch_a[-1], mu_1 = init_mu,mu_2 = minibatch_mu[-1],threshold=100)[0] and ~initseq:
            bkps.append(i)
            initseq = True
            del model
            continue
bkps.append(N)
    ...
    #如果参差中心不同并且上一个minbatch不是切换点
        #对于不同的集群中心，采用CUSUM算法进行切换点检测，
        # if center_compare(minibatch_mu[-1], minibatch_mu[-2],threshold = 5)[0] and ~initseq:
        #     bkps.append(min(i, len(X)))
        #     initseq = True
        #     del model
        #     continue
# %%
