...
def add_spur_PRI(PRI_signal, spur_ratio):
    '''
    添加虚假脉冲
    mode可以选择pulse_density或者pulse_ratio
    '''
    TOA_signal = PRI2TOA(PRI_signal)
    #计算虚假脉冲个数
    ...
    TOA_signal.sort()
    PRI_spur = TOA2PRI(TOA_signal)
    return PRI_spur

def add_miss_PRI(PRI_signal, miss_ratio):
    '''
    缺失脉冲
    miss_ratio 为 缺失脉冲的比例
    '''
    TOA_signal, init_TOA = list(PRI2TOA(PRI_signal))
    #计算缺失脉冲个数
    ...
    PRI_miss = TOA2PRI(TOA_signal_miss, )
    return PRI_miss
