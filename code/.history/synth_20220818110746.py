...
if __name__=="__main__":
    ...
    #数据生成
    N, K_grnd, K = 100, 5, 10
    X1, _, Z, mu_grd = Non_ideal_HMM(N, K_grnd, mu_sd_factor= 2, spur_ratio=0., miss_ratio=0,type='stagger')
    ...
    plt.scatter(range(len(X1)), X1, c=col,marker='*',s=50)
    plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
    plt.margins(0,0)

    plt.savefig("genworkmode/stagger_color.png", transparent=True, dpi=300, pad_inches = 0,format='png')
    plt.show()
