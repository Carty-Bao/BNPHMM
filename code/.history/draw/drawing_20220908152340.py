...
#%% 
bkps_dic = np.load('../dataset/D2/bkps_dic.npy', allow_pickle=True)
#%%
def calculate_FA_MD_DD(bkps, bkps_truth):
   FA, MD, DD = 0,0,0
   for bkp in bkps:
      dis = abs(bkp - np.array(bkps_truth))
      obj_index = np.argmin(dis)
      diff = bkp - bkps_truth[obj_index]
      if diff >= 0:
         DD += diff
      else:
         FA += 1
   ...
   for bkp_truth in bkps_truth:
      ...
      res = list(filter(lambda x: x >= 0, Detect))
...
bkp_num = 0
bkp_truth_num = 0
for bkps in bkps_dic:
   FA, MD, DD = calculate_FA_MD_DD(bkps, bkps_truth)
   bkp_num += len(bkps)
   bkp_truth_num += len(bkp_truth_num)
   FAR = FA / len(bkps)
   MDR = MD / len(bkps)
   ADD = DD / len(bkps_truth)
