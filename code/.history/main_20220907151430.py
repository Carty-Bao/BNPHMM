...
D1 = np.load('dataset/D7_unideal010.npy', allow_pickle=True)[0]
bkps_truth = np.load('dataset/D7_unideal005.npy', allow_pickle=True)[1]
X = np.array([D1[:,0]]).T
Z = np.array([D1[:,1]]).T

# 定义基于chi2 GLR的在线切换点检测器
CF_p = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, threshold=250)
...
while i < N:
    ...
    if initseq==True:
        ...
        mean, expZ, expA = model.del_irr(0.015)
        scor = CF_p.update(mean)
        minibatch_mu.append(mean)
        minibatch_a.append(expA)
        minibatch_z.append(expZ)
