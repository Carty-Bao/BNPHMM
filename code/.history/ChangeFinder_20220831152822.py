...
class CUSUM_BNP_HMM(object):
    def __init__(self, bkps=[], mean=[], var=[], para_known=False, threshold=20):
        ...
        #在中间列表中用来计数
        self.index = 0
        #generalized hypergeometric function参数
        self.r = 1
        #存放真实的均值
        self.average = []
...
if __name__ == '__main__':
    ...
    test_data1 = np.load('dataset/Gauss_PRI1.npy')[:400]
    test_data2 = np.load('dataset/Gauss_PRI2.npy')[:400]
    ...
    start = time.time()
#################################################################   CUSUM_BNP-HMM    ###################################################
    CF = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, threshold=140)
