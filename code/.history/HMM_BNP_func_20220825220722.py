...
class DP_GMM(object):
    ...
    def __init__(self,X,K,Z=None,agile=False):
        ...
        self._hyperparams = {'beta0':(1e-40), #均值的偏差，越小，GMM范围越大
                        ...
                        'W0':(1e0)*np.eye(self.XDim), #协方差的先验，越大，方差越小
                        'alpha':1., #DP-GMM的DP参数
    ...
    def update(self,minibatch,label=None, share=True, add_one = None):
        self.X = minibatch
        self.N, self.XDim = self.X.shape
        if add_one is not None:
            ...
            new_z = np.expand_dims(new_z, axis=0)

            self.X = np.append(self.X, new_x, axis=0)
            self.Z = np.append(self.X, new_z, axis=0)
            self.N, self.XDim = self.X.shape
            ...
            new_s[:,:] = self.exp_s[:,:] / self.exp_s[:,:].sum()
            new_s = np.expand_dims(new_s, axis=0)
            self.exp_s = np.append()
            
            self.exp_s = np.append(self.exp_s, np.expand_dims(np.random.dirichlet(np.ones(self._K)),axis=0),axis=0)
            self.exp_s = np.array([np.random.uniform(0,100,(self._K,self._K)) for _ in range(self.N)])#初始化状态转移矩阵
            for n in range(self.N): 
                self.exp_s[n,:,:] = self.exp_s[n,:,:] / self.exp_s[n,:,:].sum()#归一化
