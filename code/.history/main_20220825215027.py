...
#%%
a=np.array([np.random.dirichlet(np.ones(5)) for _ in range(10)])
b = np.random.dirichlet(np.ones(5))
...
for i in range(N):
    if initseq==True:
        ...
        model.update(minibatch=minibatch,label = Znibatch,share = True,add_one = [X[i],Z[i]])
        mean, Z, A = model.del_irr(threshold=0.4)
        minibatch_mu.append(mean)
        minibatch_a.append(A)
        minibatch_z.append(Z)
