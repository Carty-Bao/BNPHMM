...
class DP_GMM(object):
    ...
    def __init__(self,X, Y, K,XDim):
        ...
        self._hyperparams = {'beta0':(1e-20), #variance of mean (smaller: broader the means)
                            ...
                            'm0':np.random.randint(low=np.min(X), high=np.max(X),size=1).astype(float)
    ...
    def update_m_sigma(self,randInit=0):
        ...
        NK = self.exp_z.sum(axis=0)#Bishop Eqn 10.51
        self._NK = NK
        vk = v0 + NK + 1#Bishop Eqn 10.63
        self._vk = vk
        xd = self.calc_xd()#Bishop Eqn 10.52
        self._xd = xd
        S = self.calc_S()#Bishop Eqn 10.53
        self._S = S
        betak = beta0 + NK#Bishop Eqn 10.60
        self._betak = betak

        self._m = self.calc_mu(XDim,beta0)#Bishop Eqn 10.61
        self._W = self.calc_W(XDim,beta0) #Bishop Eqn 10.52
    ...
    def loglik(self):
        ...
        prior_lik = np.dot(self.Y,self._exp_ln_ctk)#eqn 10.66 Bishop
        return np.array([ln_lik,prior_lik]).sum(axis=0)

    def _eInvc(self,W,vk,XDim,K):#eqn 10.65 Bishop
    ...
    def _eDiffMu(self,X,XDim,NK,betak,m,W,xd,vk,N,K):#eqn 10.64 Bishop,计算对数似然
        Mu = np.zeros((N,K))
        A = XDim / betak
        for k in range(K):
            B0 = (X - m[k,:]).T
    ...
    def calc_S(self):
        ...
        xd = self._xd
        K = self._K
        (N,K)=np.shape(self.exp_z)
        (N1,XDim)=np.shape(self.X)
        assert N==N1
