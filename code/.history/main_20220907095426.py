...
#%%
curve = np.vstack((FARs,ADDs))
# np.savetxt("curves/FSS_020.csv", curve, delimiter=",") 
...
bank = ['r','g','b','orange','brown','purple','rosybrown','black','grey']
...
# bkps_truth = np.load('dataset/D7_unideal.npy')[1]
X = np.array([D1[:,0]]).T
Z = np.array([D1[:,1]]).T
...
while i < N:
    ...
    if initseq==True:
        ...
        #     continue
bkps.append(N)
FAR, ADD = calculate_FAR_ADD(bkps, [150,300,450])
print("FAR:",FAR)
print("ADD:", ADD)
...
for index in range(1,len(bkps)):
    plt.plot(range(bkps[index-1]+1, bkps[index]),X[bkps[index-1]+1: bkps[index]], marker="*", color=bank[index])
# plt.show()
# plt.savefig("draw/D7_unideal.pdf",dpi=300)
...
D1 = np.load('dataset/D7_unideal005.npy', allow_pickle=True)[0]
bkps_truth = np.load('dataset/D7_unideal005.npy', allow_pickle=True)[1]
...
rest = 10
start = time.time()
CF = ChangeFinder(r=0.06, order=2, smooth=7, outlier = False)
