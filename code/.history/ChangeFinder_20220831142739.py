...
class CUSUM_BNP_HMM(object):
    ...
    def calculate_threshold(self, theta_1, theta_2):
        length = len(self.ts_temp)
        threshold = (-2 * (theta_2 - theta_1) * sum(self.ts_temp) + length * (np.square(theta_2) - np.square(theta_1)) )/8
        return threshold*2

    def calculate_log_likelihood_ratio(self, ave, x):
        length = len(self.ts_temp)
        LLR = (-2 * (x - ave) * sum(self.ts_temp) + length * (np.square(x) - np.square(ave))) / 8
        return LLR
    
    def lnG(self, d, z, order=3):
        """
    ...
    def calculateS_kt(self, ave, pulse):
        S_kt = 0
        length = len(self.ts_temp)
        b2 = self.b_square(self.theta_0, pulse)
        chi2 = self.chi_square(ave, self.theta_0, ave)
        S_kt += -length * b2
        S_kt += self.lnG(self.r/2, b2 * np.power(length, 2) * chi2)
    ...
    def update(self, x):
        ...
        for pulse in self.ts_temp:
            ...
            ave = np.mean(buffer)
            LLR = self.calculate_log_likelihood_ratio(ave, pulse)
            self.s_kt.append(LLR)
        
        maxS_kt = np.max(np.cumsum(self.s_kt[::-1]))

        threshold = self.calculate_threshold(ave, ave+self.changepoint_th)
        if maxS_kt > (threshold):
...
#%%
CF = CUSUM_BNP_HMM(bkps=[], mean=[], var=[], para_known=False, changepoint_th=140)
theta_0 = np.array([100,110,120,130])
theta = np.array([101,111,121,128])
CF.b_square
#%%
