...
class DP_GMM(object):
    ...
    def init_q_param(self):
        #初始化
        self.m0 = np.random.randint(low=np.min(X), high=np.max(X),size=1).astype(float)
        self._W = []
        for k in range(K): 
            self._W.append(self._hyperparams['W0']) #初始化协方差矩阵
        
        self.exp_z = np.array([np.random.dirichlet(np.ones(K)) for _ in range(N)])
        self.tau_pi0, self.tau_pi1 = np.ones(K), np.ones(K)
    ...
    def fit(self):
        ...
        alpha_pi = self._hyperparams['alpha_pi'] #hyperparam for initial state DP
        alpha_a  = self._hyperparams['alpha_a'] #hyperparam for transition DP
        self.alpha_c = self._hyperparams['alpha_c'] #hyperparam for DPMM prior
        ...
        while (itr<10) or (itr<200 and diff>thre):           
            ...
            self.mixMPi(alpha_pi,K)#Blei Equ.18 Equ.19
            model.m(X,Y)
            ...
            ln_obs_lik = model.loglik(X,Y)
            exp_ln_pi = self.ePi(K)
            #find expected values of latent variables:
            self.exp_z = self.mixEZ(ln_obs_lik, exp_ln_pi, N, K) #mixture model estimation of Z
    ...
    def m(self,X,Y,randInit=0):
        ...
        vk = v0 + NK + 1
        xd = self._mXd(self.exp_z,X)
        S = self._mS(self.exp_z,X,xd,NK)
        betak = beta0 + NK
        ...
        self._vk = vk
        self._betak = betak
        tau_ctk = self.alpha_c + np.dot(Y.T, self.exp_z)
    ...
    def loglik(self,X,Y):
        ...
        exp_diff_mu = self._eDiffMu(X,XDim,self._NK,self._betak,self._m,self._W,self._xd,self._vk,N,K) #eqn 10.64 Bishop,计算对数似然
        exp_invc = self._eInvc(self._W,self._vk,XDim,K) #eqn 10.65 Bishop
        ln_lik = 0.5*exp_invc - 0.5*exp_diff_mu

        prior_lik = np.dot(Y,self._exp_ln_ctk)
    ...
    def mixMPi(self,alpha_pi,K):               #Blei Equ.18 Equ.19
        ...
        for k in range(K):
            self.tau_pi0[k] = alpha_pi + self.exp_z[:,k+1:].sum() #hyperparam for this component NOT explaining the data
            self.tau_pi1[k] = 1. + self.exp_z[:,k].sum() #hyperparam for this component explaining the data
    ...
    def mixEZ(self, ln_obs_lik, exp_ln_pi, N, K):#Blei Equ 22
