...
class DP_GMM(object):
    ...
    def fit(self):
        ...
        while (itr<10) or (itr<200 and diff>thre):           
            ...
            tau_pi0,tau_pi1 = self.mixMPi(alpha_pi,self.exp_z,K)#Blei Equ.18 Equ.19
            model.m(X,Y, self.exp_z)
            ...
            exp_ln_pi = self.ePi(tau_pi0,tau_pi1,K)
    ...
    def m(self,X,Y,exp_z,randInit=0):
    ...
    def mixMPi(self,alpha_pi,exp_p,K):               #Blei Equ.18 Equ.19
        ...
        for k in range(K):
            tau_pi0[k] = alpha_pi + self.exp_z[:,k+1:].sum() #hyperparam for this component NOT explaining the data
            tau_pi1[k] = 1. + self.exp_z[:,k].sum() #hyperparam for this component explaining the data
        return tau_pi0, tau_pi1
    
    def ePi(self,tau_pi0,tau_pi1,K):
