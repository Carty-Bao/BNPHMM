...
if __name__ == '__main__':
    ...
    # test_path = "dataset/D7_unideal001/"
    # D1 = np.load(test_path + str(15)+'.npy',allow_pickle = True)
    # X = np.array([D1[0][:,0]]).T
    # Z = np.array([D1[0][:,1]]).T
    # bkps_truth = D1[1]
    # # X = np.array([D1[:,0]]).T
    ...
    # bkps = BNPHMMCUSUM(X, Z, CUSUM_threshold=10, unideal_threshold=0.01)
    # # print(bkps_truth)
    # bkps_dic.append(bkps)
    ...
    for i in range(64):
        #读取数据+数据整形
        data_path = "dataset/D7_unideal005/"
        ...
        bkps_truth_dic.append(bkps_truth)
        #推理
        p.apply_async(BNPHMMCUSUM, args=(X,Z,100,0.01,),callback=add_bkps)
    p.close()
    p.join()
    #保存
    np.save(data_path + 'result/bkps_dic_BNPHMMCUSUM.npy',bkps_dic)
    np.save(data_path + 'result/bkps_truth_dic_BNPHMMCUSUM.npy',bkps_truth_dic)
    dataset_eva(bkps_dic, bkps_truth_dic)
