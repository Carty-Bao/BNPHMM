...
if __name__ == '__main__':
    bkps_dic = []
    bkps_truth_dic = []
    thresholds = np.linspace(0.1,100,20)
    ...
    for b_index, batch in enumerate(batches):
        for th_index, th in enumerate(thresholds):
            ...
            p.close()
            p.join()
            np.save('curves/D2_BNPHMMF/bkps_dic_'+str(b_index)+'_'+str(th_index)+'.npy',bkps_dic)
            np.save('curves/D2_BNPHMMF/bkps_truth_dic_'+str(b_index)+'_'+str(th_index)+'.npy',bkps_truth_dic)
